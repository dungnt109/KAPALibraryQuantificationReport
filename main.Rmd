...
\fancypagestyle{plain}{%
  ...
  \fancyhf{}%
  \fancyfoot[C]{\footnotesize Page \thepage\, of\, \pageref*{LastPage} page `r analysis_type` Generated at `r generated_date` by CFX96 Relative Quantification Pipeline `r pipeline_version`}
...
for (i in 1:nrow(runs)) {
	...
	if (analysis_type == "KAPA Library Quantification"){

		if (input_type == "Standard Curve") {
            		result = render_template1(run, summary, globalVar)
	    		summary <- result$summary 
		} else if (input_type == "Standard Curve (with samples)") {

		} else if (input_type == "Import"){
            		result = render_template3(run, summary, globalVar)
	    		summary <- result$summary
		} 
	} else if (analysis_type == "Albumin Quantification") {
...
if (analysis_type == "Albumin Quantification"){ 

	cat(knitr::knit_child("Albumin_summary.Rmd", quiet = TRUE))

} else {

	cat(knitr::knit_child("KAPA_summary.Rmd", quiet = TRUE))
